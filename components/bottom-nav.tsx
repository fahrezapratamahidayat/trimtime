...
import { CheckCircle2, Home, Scissors, Sparkles, User } from "lucide-react";

import { cn } from "@/lib/utils";

export const appNavItems = [
  { href: "/homepage", label: "Beranda", icon: Home },
  { href: "/orders", label: "Pesanan", icon: Scissors },
  { href: "/promo", label: "Promo", icon: Sparkles },
...
export function BottomNav({ className, ...props }: BottomNavProps) {
  ...
  const allowedPrefixes = appNavItems.map(({ href }) => href) as ReadonlyArray<
    (typeof appNavItems)[number]["href"]
  >;
    ...
    return (
      ...
      >
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-5 py-4 sm:px-6">
          ...
          <Link href={`/booking/${barbershopId}`} className="shrink-0">
